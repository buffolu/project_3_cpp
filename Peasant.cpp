...
void Peasant::update() {
     if(getState() == ON_DUTY)
     {
        ...
        if(getLocation() == m_castle->getLocation() && !loading) //peasant reached the castle, unload and stop
        {
...
void Peasant::stop() { // beside the change of state, it also makes sense to use it when killing the farmer
    ...
    m_castle.reset();
    m_farm.reset();
    while(!farms.empty()) farms.pop();
    while(!castles.empty()) castles.pop();
